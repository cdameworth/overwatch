...
const { parseTerraform, parseMultipleTerraform } = require('./utils/terraform-parser');
const { parseMultipleTerraformFromGitHub } = require('./utils/terraform-github-parser');
const { inferDependencies, inferCrossApplicationDependencies } = require('./utils/dependency-engine');
const GitHubService = require('./services/github-service');
const AuthService = require('./services/auth-service');
const EnterpriseRepositoryService = require('./services/enterprise-repository-service');
...
// Load GitHub connections configuration
const githubConnections = require('./config/github-connections.json');
...
app.use(cors({
  origin: [
    ...
    'http://localhost:5000', 
    'http://127.0.0.1:3000',
    'http://127.0.0.1:51254',  // Add the actual frontend port
...
// GitHub connections management endpoints
app.get('/api/github/connections', (req, res) => {
  try {
    // Return connections without sensitive data
    const safeConnections = githubConnections.connections.map(conn => ({
      id: conn.id,
      name: conn.name,
      type: conn.type,
      owner: conn.owner,
...
app.get('/api/github/connections/:connectionId/repositories', (req, res) => {
  try {
    const { connectionId } = req.params;
    const connection = githubConnections.connections.find(c => c.id === connectionId);
    
    if (!connection) {
      return res.status(404).json({ error: 'Connection not found' });
    }
    
    res.json({ repositories: connection.repositories });
  } catch (error) {
    console.error('Error loading repositories for connection:', error);
    res.status(500).json({ error: 'Failed to load repositories' });
  }
});

// Enhanced GitHub repository parsing with connection support
app.post('/api/parse-github-connection', async (req, res) => {
  const { connectionId, repositoryName, branch } = req.body;
  
  if (!connectionId || !repositoryName) {
    return res.status(400).json({ error: 'Connection ID and repository name are required' });
  }
  ...
  try {
    ...
    // Find the repository in the connection
    const repository = connection.repositories.find(r => r.name === repositoryName);
    if (!repository) {
      return res.status(404).json({ error: 'Repository not found in connection' });
    }
    ...
    console.log(`Parsing GitHub repository via connection: ${owner}/${repo} (branch: ${selectedBranch})`);
    
    // Use the existing GitHub parsing logic
    ...
    if (connection.accessToken) {
      ...
      if (token.startsWith('env:')) {
        const envVar = token.replace('env:', '');
    ...
    console.log(`Repository tree response status: ${treeResponse.status}`);
    console.log(`Total files in repository: ${treeResponse.data.tree.length}`);
    
    // Log all files for debugging
    const allFiles = treeResponse.data.tree.map(item => item.path);
    console.log(`All files in repository:`, allFiles.slice(0, 20)); // First 20 files
    
    // Find .tf files
    const terraformFiles = treeResponse.data.tree
      .filter(item => item.type === 'blob' && item.path.endsWith('.tf'));
    
    console.log(`Found ${terraformFiles.length} Terraform files:`, terraformFiles.map(f => f.path));
    
    // Only slice after logging for debugging
    const limitedTerraformFiles = terraformFiles.slice(0, 10); // Limit to first 10 files for demo
    
    if (limitedTerraformFiles.length === 0) {
      return res.json({
        resources: { resource: {} },
        dependencies: [],
        groups: {},
        ...
        connection: { id: connectionId, name: connection.name, owner, repo, branch: selectedBranch }
      });
    }
    
    console.log(`Processing ${limitedTerraformFiles.length} Terraform files (limited from ${terraformFiles.length} total)`);
    ...
    for (const file of limitedTerraformFiles) {
      try {
        // Get file content
        const fileUrl = `https://api.github.com/repos/${owner}/${repo}/contents/${file.path}?ref=${selectedBranch}`;
        ...
        const fileName = pathParts[pathParts.length - 1].replace('.tf', '');
        const appName = pathParts.length > 1 ? pathParts.slice(0, -1).join('-') + '-' + fileName : fileName;
        const appId = `github-${appName}`;
        
        // Parse the Terraform file with error handling
        const { parseTerraform } = require('./utils/terraform-parser');
        console.log(`Parsing file: ${file.path} (${content.length} characters)`);
        
        let parsed;
        try {
          parsed = parseTerraform(content, appId);
          console.log(`Successfully parsed ${file.path}`);
          if (parsed && parsed.resources) {
            const resourceCount = Object.keys(parsed.resources.resource || {}).length;
            console.log(`  Found ${resourceCount} resources in ${file.path}`);
            console.log(`  Resource types:`, Object.keys(parsed.resources.resource || {}).slice(0, 5));
          } else {
            console.log(`  No resources found in ${file.path}`);
          }
        } catch (parseError) {
          console.error(`Error parsing Terraform file ${file.path}:`, parseError.message);
          console.error(`Parse error stack:`, parseError.stack);
          // Continue with next file instead of crashing
          continue;
        }
        
        if (parsed && parsed.resources && Object.keys(parsed.resources.resource).length > 0) {
          // Create app object
          const app = {
            id: appId,
            name: appName,
          ...
          // Merge resources
          Object.keys(parsed.resources.resource).forEach(resourceType => {
            if (!allResources.resource[resourceType]) {
              allResources.resource[resourceType] = {};
            }
            Object.assign(allResources.resource[resourceType], parsed.resources.resource[resourceType]);
          });
          
          // Add to groups
          Object.keys(parsed.resources.resource).forEach(resourceType => {
            Object.keys(parsed.resources.resource[resourceType]).forEach(resourceName => {
              const resourceKey = `${resourceType}.${appName}_${resourceName}`;
              allGroups[resourceKey] = {
                appId,
                appName,
                source: 'github',
                repository: {
    ...
    // Infer dependencies
    const { inferDependencies } = require('./utils/dependency-engine');
    allDependencies = inferDependencies(allResources);
    ...
    const response = {
      ...
      apps: applications,
      dataSource: 'github',
      repository: { owner, repo, branch: selectedBranch },
    ...
    console.error('Error parsing GitHub repository via connection:', error);
    res.status(500).json({ 
